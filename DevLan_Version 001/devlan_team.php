<?php include("assets/_partials/head.php");?>
<body>
<div class="page-loader">
	<img src="{{ url('assets/img/loader.gif') }}" alt="">
</div>
<!-- Header
================================================== -->
<header id="header">
<div id="mega-menu" class="header header-sticky primary-menu icons-no default-skin zoomIn align-right">
	<div class="container">
		<div class="row">
            <!---Navigation Bar-->
			<div class="row">
			<?php include("assets/_partials/navbar.php");?>
			<!-- end nav -->
		</div>
			<!-- end nav -->
		</div>
		<!-- end .row -->
	</div>
	<!-- end .container -->
</div>
<!-- end .header -->
</header>
<!-- Parallax Section
================================================== -->
<section class="pagetitle parallax parallax-image" style="background-image:url(https://www.themepush.com/demo-zolix/wp-content/uploads/sites/3/2014/07/beachppl.jpg);">
<div class="wrapsection">
	<div class="overlay" style="background:#303543;opacity:0.4;">
	</div>
	<div class="container">
		<div class="parallax-content">
			<div class="block2 text-center max80" style="padding:120px 0;color:#fff;">
				<h3>
				<span class="text1 big wow bounceIn" data-wow-delay="0s" data-wow-duration="1s">
				<b>The Team Behid DevLan</b>
				</span>
				</h3>
				<h2>
				<span class="text2 big wow zoomIn" data-wow-delay="0.4s" data-wow-duration="2s">
				 </span>
				</h2>
				<a href="#le_gang" class="downarrowpoint wow zoomIn goscroll"><i class="fa fa-angle-double-down"></i></a>
			</div>
		</div>
	</div>
</div>
</section>
<!-- Gallery Full
================================================== -->
<section id="le_gang" class="page-wrapper gray wow zoomIn">
	<div class="container">
	<h2 class="title">Team</h2>
	<p class="tagline">
		 Le Gang Behind Dev Lan 
	</p>

	<div class="row team-wrap">
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/mark.jpg" alt="Markov">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Markov</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Mark</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/alonso.jpg" alt="Anatoli">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Anatoli</span><br/>
								<i>Dev Lan<br/>Software Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Antony</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/el efe.jpg" alt="Juan">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">El Jefe'</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Juan</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
	</div>

	<div class="row team-wrap">
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/will_i_am.jpg" alt="Thannos">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Will_ I_Am</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>William</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/doris.jpg" alt="Doris">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;"></span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Doris</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/matt.jpg" alt="Mustang 001">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Mart </span><br/>
								<i>Dev Lan<br/>Software Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Martin</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
	</div>

	<div class="row team-wrap">
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/sejo.jpg" alt="Sejo">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Sejo Rikel</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Joseph</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/pptx.jpg" alt="pptx">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Bergham</span><br/>
								<i>Dev Lan<br/>Software Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Perpetual</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		
		
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/sam.jpg" alt="">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Chebweche</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Samuel</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div> 
	</div>

	<div class="row team-wrap">
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/noel.jpg" alt="">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Noel Illinois</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Noel</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/ayub.jpg" alt="">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">Ayub_</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Ayub</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div>
		
		
		<div class="col-md-4 col-sm-4 col-xs-12">
			<div class="team-boxes">
				<div class="team-thumb overlay-image view-overlay">
					<img src="assets/img/demo/kevv.jpg" alt="">
					<div class="clear">
					</div>
					<div class="mask team_quote">
						<div class="port-zoom-link">
							<p>
								<span class="accentcolor" style="font-weight:700;">JahYah</span><br/>
								<i>Dev Lan<br/>Network Engineer</i>
							</p>
						</div>
					</div>
				</div>
				<div class="team-info">
					<h2>Kelvin</h2>
					
				</div>
				<div class="separator">
				</div>
				
			</div>
		</div> 
		
		<div class="row team-wrap">		
	</div>

</div>
</section>
<!-- Footer
================================================== -->
<?php include("assets/_partials/footer.php");?>
<!-- JavaScript
================================================== -->
<?php include("assets/_partials/script.php");?>

<script>
$(document).ready(function(){
	$('.gallery').featherlightGallery({
		gallery: {
			fadeIn: 0,
			fadeOut: 0
		},
		openSpeed:    500,
		closeSpeed:   500
	});
});
</script>
</body>

</html>